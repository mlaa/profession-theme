...
function get_the_image( $args = array() ) {
    ...
    if ( !isset( $image_cache[$key] ) || empty( $cache ) ) {
        ...
        /* If no image found, try using the video thumbnail if the post contains a video in the embed field. (WPZOOM customization) */
        if ( empty( $image ) ) {
            if ( !class_exists( 'WPZOOM_Video_Thumb' ) ) require_once WPZOOM_INC . '/components/video-thumb.php';
            $wpzthumburl = WPZOOM_Video_Thumb::gettheimage_url( $post_id );
            if ( $wpzthumburl !== false ) $image = $wpzthumburl;
        }
...
function get_the_image_by_scan( $args = array() ) {
    ...
    $videocode = get_post_meta($args['post_id'], 'wpzoom_post_embed_code', true);
